...
8. 開啟 `status.html`，輸入任務編號；也可選填填寫人。
9. 確認追蹤頁顯示「已完成」與 Jia 的交付回覆。

只有上述九項全部成功，才建議進入 GitHub 上架階段。
