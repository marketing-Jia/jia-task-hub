...
- 同仁填寫頁：填寫人、項目類別、工作標題、工作說明、交付日期（可選其他／待確認）、備註。
- 發任務人追蹤頁：可用任務編號查詢；忘記編號時，可用完整填寫人內容找回最近 10 筆任務。
- 獨立管理看板：不出現在公開導覽，依交付日期或 Jia 的內部排程日期排序，支援篩選、狀態更新與交付內容回覆。
- 內部排程日期：前台選擇其他／待確認時，可由 Jia 在管理看板安排，前台與追蹤頁不顯示。
- Google Sheets 後端：新資料寫入後，試算表本身也會自動依交付日期排序。
- Google Sheets 狀態欄：J 欄提供「待處理／進行中／已完成」下拉選單，直接選擇也會更新最後更新時間。
- 響應式設計：桌機與手機皆可使用。
- 任務送出成功後可一鍵複製任務編號。
...
5. 在函式選單執行一次 `setupSpreadsheet`，完成授權並確認建立 A–M 欄。
6. 點選「部署 → 新增部署作業」。
...
重新整理網站後，表單右上角會顯示「Google Sheets 已連線」。第一次有人送出需求時，後端會自動建立名為「工作需求」的工作表及欄位標題。發任務人可使用 `status.html` 搭配任務編號及選填的填寫人追蹤進度；Jia 則可在獨立的 `admin.html` 更新狀態與交付回覆，追蹤頁重新查詢時會顯示最新內容。
...
1. 先備份試算表。
2. 用新版 `google-apps-script/Code.gs` 完整取代 Apps Script 內容。
3. 手動執行一次 `setupSpreadsheet`；程式會在 E 欄加入「工作標題」，既有工作說明會保留並建立對應標題。
4. 到「部署 → 管理部署作業」，編輯原部署並選擇「新版本」後重新部署。
5. 重新測試管理看板的狀態更新與追蹤頁查詢。
...
目前管理看板網址本身沒有獨立密碼；誰能開啟 `admin.html`，誰就能看到需求並更新狀態。追蹤頁可用任務編號或完整填寫人內容找回任務，因此請避免在工作說明與標題放入敏感資訊。若內容含敏感資料，請至少採用以下其中一種方式：
